---
description: Verifies that the PreProd SarFrance blog page is accessible and contains expected content
on:
  workflow_dispatch:
...
- **Target URL**: https://preprod.sarfrance.org/projects/agentic-workflows/
...
1. **Navigate to URL**: Use `browser_navigate` to load https://preprod.sarfrance.org/projects/agentic-workflows/
...
- **Requirement**: Final URL after redirects must match the target URL or be within the same allowed domain (preprod.sarfrance.org)
...
#### URL Redirect Check
- **Expected**: preprod.sarfrance.org domain
- **Actual**: [final URL]
- **Status**: [✅ PASS / ❌ FAIL]
...
4. **Check Site**: Verify if there are broader issues with the preprod.sarfrance.org site
