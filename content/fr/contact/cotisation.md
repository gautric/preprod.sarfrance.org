---
title: "Cotisation et don"
description: "Paiement en ligne : cotisation, don et autres achats — SAR France"
---

## Paiement en ligne

Vous pouvez régler votre cotisation, faire un don à la Société ou régler votre participation à l'une de nos activités via le service sécurisé Payasso à l'aide du lien suivant :

**[https://www.payasso.fr/revolution-americaine/cotisation](https://www.payasso.fr/revolution-americaine/cotisation)**

## Montants

- **Cotisation annuelle** : {{< param "cotisation" >}} €
- **Frais de chancellerie** (première adhésion) : {{< param "chancellerie" >}} €
- **Première inscription** (chancellerie + cotisation) : {{< param "premiere_inscription" >}} €
- **Cotisation réduite** (moins de 35 ans et résidents étrangers) : réduction de {{< param "reduction_pct" >}}%

## Faites un don à SAR France

SAR France, association reconnue d'utilité publique, est habilitée à faire appel aux dons pour soutenir son action dans les domaines prévus par ses statuts.

## Avantage fiscal

SAR France est une association reconnue d'utilité publique. À ce titre, une réduction d'impôt de {{< param "avantage_fiscal_pct" >}}% du montant de votre cotisation ou de vos dons est accordée aux contribuables français.
